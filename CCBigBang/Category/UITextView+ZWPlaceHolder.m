...
+(void)load{
    ...
    method_exchangeImplementations(class_getInstanceMethod(self.class, @selector(setText:)),
...
- (void)zwPlaceHolder_swizzled_setText:(NSString *)text{
    NSString *prev_text = self.text;
    [self zwPlaceHolder_swizzled_setText:text];
    
    if( ![text isEqualToString:prev_text]){ [self updatePlaceHolder]; }
}
